...
class ResNet1d(nn.Sequential):
    ...
    def __init__(self, block,
        ...
        layers_tmp.append(ConcatAvgMaxPool())
        layers_tmp.append(nn.Linear(256, 128, bias=False))
        layers_tmp.append(nn.BatchNorm1d(128))
        layers_tmp.append(nn.ReLU())
        layers_tmp.append(nn.Dropout(.25))
        layers_tmp.append(nn.Linear(128, 128, bias=False))
        layers_tmp.append(nn.BatchNorm1d(128))
        layers_tmp.append(nn.ReLU())
        layers_tmp.append(nn.Dropout(.5))
        layers_tmp.append(nn.Linear(128, num_classes))
...
def resnet_ptbxl_wang(in_channels, num_classes):
    kernel_size = 5
    ...
    return ResNet1d(BasicBlock1d, [1, 1, 1],
                    kernel_size=kernel_size, input_channels=input_channels, num_classes=num_classes,
                    kernel_size_stem=kernel_size_stem, stride_stem=stride_stem, pooling_stem=pooling_stem,
                    inplanes=inplanes)
