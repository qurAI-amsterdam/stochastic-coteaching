...
def conv1x1(in_planes, out_planes, stride=1, bias=True):
    """1x1 convolution"""
    return nn.Conv1d(in_planes, out_planes, kernel_size=1, stride=stride, bias=bias)

def conv5(in_planes, out_planes, stride=1, groups=1, dilation=1, bias=True):
    """5 convolution with padding"""
    pad = dilation * (5 // 2)
    return nn.Conv1d(in_planes, out_planes, kernel_size=5, stride=stride,
                     padding=pad, groups=groups, bias=bias, dilation=dilation)
...
class BobBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=nn.BatchNorm1d, bias=False):
        super().__init__()
        self.dropout = nn.Dropout(p=0.3)
        self.normal_block = nn.Sequential(conv5(inplanes, planes, stride=stride, bias=bias),
                                          norm_layer(planes),
                                          AF(inplace=True),
                                          conv5(planes, planes, bias=bias),
                                          norm_layer(planes),

                                          # AF(inplace=True),
                                          )

        self.dilated_block = nn.Sequential(conv5(inplanes, planes, stride=stride, dilation=dilation, bias=bias),
                                           norm_layer(planes),
                                           AF(inplace=True),
                                           conv5(planes, planes, bias=bias),
                                           norm_layer(planes),
...
class ResNet(nn.Module):
    def __init__(self, block, layers, dilation, kernels, num_channels=1, num_classes=4, zero_init_residual=False,
        ...
        self.fc = nn.Sequential(
                # nn.Linear(kernels[-1]*4*2, kernels[-1], bias=False),
                # nn.BatchNorm1d(kernels[-1]),
                # nn.LeakyReLU(inplace=True),
                # # nn.Dropout(p=0.5),
                # nn.Linear(kernels[-1], kernels[-1], bias=False),
                # nn.BatchNorm1d(kernels[-1]),
                # nn.LeakyReLU(inplace=True),
                # nn.Dropout(p=0.5),
                nn.Linear(kernels[-1]*4, num_classes))
    ...
    def _forward_impl(self, x):
        ...
        x = self.avgpool(x)
        # x = self.maxpool(x)
        # x = torch.cat((self.avgpool(x), self.maxpool(x)), 1)
...
def resnet_ecg_bob(**kwargs):
    ...
    if norm_layer == 'batchnorm':
        ...
        raise RuntimeError('norm_layer should be None or batchnorm')
    model = ResNet(BobBlock, [2, 2, 2, 2, 2, 2, 2, 2], [1, 2, 4, 8, 16, 32, 64, 128], [16, 16,  32, 32, 64, 64, 128, 128], norm_layer=norm_layer, **kwargs)
    return model

def resnet_large_ecg_bob(**kwargs):
    norm_layer = kwargs.pop('norm_layer', 'batchnorm')
    if norm_layer == 'batchnorm':
        norm_layer = nn.BatchNorm1d
    elif norm_layer == None:
        ...
        raise RuntimeError('norm_layer should be None or batchnorm')
    model = ResNet(BobBlock, [2, 2, 2, 2, 2, 2, 2, 2], [1, 2, 4, 8, 16, 32, 64, 128], [32, 32,  64, 64, 128, 128, 256, 256], norm_layer=norm_layer, **kwargs)
    # model = ResNet(BobBlock, [2, 2, 2, 2, 2, 2, 2, 2], [1, 2, 4, 8, 16, 32, 64, 128],
    #                [32, 32, 32, 32, 32, 32, 32, 32], norm_layer=norm_layer, **kwargs)
    return model
