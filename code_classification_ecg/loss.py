...
class StochasticCoTeachingMLLoss:
    def __init__(self, alpha, beta, max_iters, tp_gradual, delay=0, exponent=1, clip=(0.01, 0.99), Loss=nn.BCEWithLogitsLoss):
        maxval = 1
        rate_schedule = np.ones(max_iters) * maxval
        # rate_schedule[:tp_gradual] = np.linspace(0, maxval ** exponent, tp_gradual)
        rate_schedule[:delay] = 0
        ...
        # self.output_fn = lambda x: F.softmax(x, self._axis)
    ...
    def get_probas(self, logits, y):
        ...
        Get probabilities from logits of classes indicated by y.
        '''
        all_probabilities = self.output_fn(logits)
        class_probabilities = 1. - torch.abs(y - all_probabilities)
        return class_probabilities
    ...
    def __call__(self, logits_1, logits_2, y, *args, **kwargs):
        with torch.no_grad():
            ...
            self.fraction_reject_1 = (1. - (mask_1.sum() / (np.product(mask_1.shape)))).cpu().numpy()
            self.fraction_reject_2 = (1. - (mask_2.sum() / (np.product(mask_2.shape)))).cpu().numpy()
